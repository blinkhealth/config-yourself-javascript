const password = {
  ciphertext: Buffer.from('g6OUqmMFySN1w2xxrFlIwIUM/dcCws+eNuchM1wZL/p6vriOTFDuhVW6qF1I/fWKkbHfQIja', 'base64'),
  crypto: {
    provider: 'password',
    key: `N91K3dD7T4fPZRTCKm6phxkKIBqvPDo5sDlDN758vdeHHDNcDoxITpVZ4ZozFwSjOwBHsVVBgJS7lQnUO/uo0wCjGsnc5FiPJouWkiLez4GqH5BdXp6FVhmTSLA=`,
  },
  secrets: {
    password: 'password',
  },
}

const kms = {
  ciphertext: Buffer.from('secret', 'utf8'),
  crypto: {
    provider: 'kms',
    key: 'arn:aws:kms:us-east-1:000000000000:key/00000000-0000-0000-0000-000000000000',
  },
  secrets: {
    un: 'used',
  },
}

const gpg = {
  ciphertext: Buffer.from('sibTqX8Fnb+qPwtjNWHAYhcdDbdrtz1CWtnAoSiDO6+yhrcWe6pyvOB6l+2+K04YKre1GJKL', 'base64'),
  crypto: {
    provider: 'gpg',
    key: `-----BEGIN PGP MESSAGE-----


=OhLv
-----END PGP MESSAGE-----
`,
  },
  secrets: {
    privateKey: `-----BEGIN PGP PRIVATE KEY BLOCK-----


4DjkMu6uv8gq2f6a9yu1QtdzbaX+XE4wkCBMENnI
=if4J
-----END PGP PRIVATE KEY BLOCK-----
`,
  },
}
;(global as any).fixtures = {
  gpg,
  kms,
  password,
}
